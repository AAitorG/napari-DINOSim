...
# code from https://github.com/BiaPyX/BiaPy/blob/master/biapy/data/data_2D_manipulation.py
...
def merge_data_with_overlap(
    ...
    last_y = (
        0
        if crops_per_y == 1
        else (((crops_per_y - 1) * step_y) + pad_input_shape[1])
        - padded_data_shape[0]
    )
    ovy_per_block = last_y // (crops_per_y - 1) if crops_per_y > 1 else 0
    step_y -= ovy_per_block
    ...
    last_x = (
        0
        if crops_per_x == 1
        else (((crops_per_x - 1) * step_x) + pad_input_shape[2])
        - padded_data_shape[1]
    )
    ovx_per_block = last_x // (crops_per_x - 1) if crops_per_x > 1 else 0
    step_x -= ovx_per_block
    ...
    for z in range(original_shape[0]):
        for y in range(crops_per_y):
            for x in range(crops_per_x):
                d_y = (
                    0
                    if (y * step_y + data.shape[1]) < original_shape[1]
                    else last_y
                )
                d_x = (
                    0
                    if (x * step_x + data.shape[2]) < original_shape[2]
                    else last_x
                )
    ...
    merged_data = np.true_divide(merged_data, ov_map_counter).astype(
        data.dtype
    )
    if data_mask is not None:
        merged_data_mask = np.true_divide(
            merged_data_mask, ov_map_counter
        ).astype(data_mask.dtype)

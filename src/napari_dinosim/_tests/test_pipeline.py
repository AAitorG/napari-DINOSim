...
class MockDINOv2Model:
    def forward_features(self, x):
        ...
        patch_tokens = torch.randn(batch_size, 518 // 14 * 518 // 14, 384)  # Example dimensions
...
def test_pipeline_initialization(pipeline):
    ...
    assert pipeline.patch_h == pipeline.patch_w == pipeline.embedding_size == 37  # 518/14
...
def test_pre_compute_embeddings(pipeline):
    ...
    pipeline.pre_compute_embeddings(
        dataset,
        overlap=(0, 0),  # Changed from (0.5, 0.5) to avoid patch multiplication
        padding=(0, 0),
    ...
    assert pipeline.embeddings.shape[0] == batch_size  # Should match input batch size
    assert pipeline.embeddings.shape[1] == pipeline.embeddings.shape[2] == 37  # 518/14
...
def test_save_load_reference(pipeline, tmp_path):
    ...
    assert torch.allclose(new_pipeline.reference_color, pipeline.reference_color)
...
def test_distance_post_processing(pipeline):
    ...
    assert processed_distances.shape == (2, 518, 518, 1)  # Should match original image dimensions

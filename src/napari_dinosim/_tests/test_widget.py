...
def mock_viewer():
    ...
    layers.__getitem__ = lambda x, key: x._list[key] if isinstance(key, int) else next((l for l in x._list if l.name == key), None)
    layers._list = []
...
def test_model_loading(widget, qt_app):
    ...
    with patch('torch.hub.load') as mock_hub:
        ...
        # Verify button state
        assert widget._load_model_btn.text == "Load New Model\n(Current Model: small)"
...
def test_reference_handling(widget):
    ...
    test_points = np.array([[10, 10], [20, 20]])  # Changed from 3D to 2D points
...
def test_cleanup(widget):
    ...
    if widget._points_layer is not None:
        widget._points_layer.events.data.disconnect(widget._update_reference_and_process)
        widget._points_layer = None
...
def test_error_handling(widget):
    ...
    with patch('torch.hub.load', side_effect=Exception("Network error")):
        ...
        assert widget.pipeline_engine == prev_pipeline  # Pipeline should remain unchanged
